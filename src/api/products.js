    ...
    try {
      ...
      if (productData.prescription_required !== undefined)
        formData.append(
          ...
          productData.prescription_required ? 1 : 0,
        );
      ...
      if (productData.perishable !== undefined)
        formData.append("perishable", productData.perishable ? 1 : 0);
      if (productData.organic_certified !== undefined)
        formData.append(
          "organic_certified",
          productData.organic_certified ? 1 : 0,
        );
      ...
      if (productData.is_returnable !== undefined)
        formData.append("is_returnable", productData.is_returnable ? 1 : 0);
      if (productData.is_refundable !== undefined)
        formData.append("is_refundable", productData.is_refundable ? 1 : 0);
      ...
      if (productData.image) {
        if (productData.image.uri) {
          const filename =
            productData.image.uri.split("/").pop() || "image.jpg";
      ...
      if (productData.images && Array.isArray(productData.images)) {
        if (productData.images.length === 0) {
          ...
          productData.images.forEach((image, index) => {
            if (image.uri) {
              const filename =
                image.uri.split("/").pop() || `image${index}.jpg`;
              const match = /\.(\w+)$/.exec(filename);
              const type = match ? `image/${match[1]}` : "image/jpeg";
    ...
    try {
      ...
      const hasNewImages =
        ...
        (Array.isArray(productData.images) &&
          productData.images.some((img) => img.uri));
