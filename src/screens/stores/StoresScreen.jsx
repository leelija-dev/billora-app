...
const StoresScreen = () => {
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      ...
      fetchStores(getUserId(), 1, { search: searchQuery });
    }, 500);
    return () => {
      if (searchTimeoutRef.current) clearTimeout(searchTimeoutRef.current);
    };
  }, [searchQuery, setFilters, fetchStores, getUserId]);
  ...
  const handleApplyFilters = (newFilters) => {
    ...
    fetchStores(getUserId(), 1, { ...filters, ...newFilters, search: searchQuery });
  };
  ...
  const handleResetFilters = () => {
    ...
    setSearchQuery("");
    setSortBy("name");
    fetchStores(getUserId(), 1, { search: "" });
  };
